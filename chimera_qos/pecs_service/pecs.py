...
class SimulatedVacuumGauge(AbstractEnvironmentDevice):
    def __init__(self, device_id: str, device_name: str, device_config: Dict[str, Any], pecs_service_callback: Optional[Callable] = None):
        super().__init__(device_id, device_name, device_config, SubSystemType.VACUUM, pecs_service_callback)
        self._current_pressure_mbar: float = float(device_config.get("initial_pressure_mbar", 1e-7))
        self._target_pressure_mbar: float = float(device_config.get("target_pressure_mbar", 1e-8))
        self._min_pressure_mbar: float = float(device_config.get("min_pressure_mbar", 1e-9))
        self._max_pressure_mbar: float = float(device_config.get("max_pressure_mbar", 1000.0)) # Atmospheric
        self._pump_on: bool = False
        self._pump_down_rate_factor: float = float(device_config.get("pump_down_rate_factor", 0.1)) # Logarithmic factor per second
        self._leak_rate_factor: float = float(device_config.get("leak_rate_factor", 0.001)) # Logarithmic factor per second if pump off
        self._last_update_time: float = time.time()

        self._register_parameter(ParameterDefinition("current_pressure_mbar", "Current vacuum pressure", "mbar", float, is_writable=False, range_min=self._min_pressure_mbar, range_max=self._max_pressure_mbar))
        self._register_parameter(ParameterDefinition("pump_status", "Pump on/off status", "", bool, is_writable=True))

    def connect(self) -> bool:
        self._is_connected = True; self._last_update_time = time.time(); self._status_message = "Connected"
        print(f"PECS.SimVacuumGauge [{self.device_id}]: Connected."); return True

    def disconnect(self) -> bool:
        self._is_connected = False; self._status_message = "Disconnected"
        print(f"PECS.SimVacuumGauge [{self.device_id}]: Disconnected."); return True

    def get_device_status_summary(self) -> Dict[str, Any]:
        self._update_simulated_state()
        return {
            "connection": "CONNECTED" if self._is_connected else "DISCONNECTED",
            "pressure_mbar": f"{self._current_pressure_mbar:.2e}",
    ...
    def _update_simulated_state(self):
        ...
        now = time.time()
        delta_time_s = now - self._last_update_time
        if delta_time_s <= 0.01: return

        if self._pump_on:
            # Exponential decay towards target (or min_pressure if target is lower)
            effective_target = max(self._target_pressure_mbar, self._min_pressure_mbar)
            if self._current_pressure_mbar > effective_target:
                 # P_new = P_old * exp(-rate * dt) approx P_old * (1 - rate * dt) for small rate*dt
                 self._current_pressure_mbar *= (1 - self._pump_down_rate_factor * delta_time_s)
                 self._current_pressure_mbar = max(self._current_pressure_mbar, self._min_pressure_mbar)
            if abs(self._current_pressure_mbar - effective_target) < 1e-12 : # effectively reached
                self._current_pressure_mbar = effective_target
        else: # Pump off, simulate slow leak up to max_pressure_mbar (atmospheric)
            if self._current_pressure_mbar < self._max_pressure_mbar:
                self._current_pressure_mbar *= (1 + self._leak_rate_factor * delta_time_s)
                self._current_pressure_mbar = min(self._current_pressure_mbar, self._max_pressure_mbar)

        self._last_update_time = now

        # Update status message based on pressure ranges
        if self._current_pressure_mbar < 1e-6: self._status_message = "High Vacuum"
        elif self._current_pressure_mbar < 1e-3: self._status_message = "Medium Vacuum"
        elif self._current_pressure_mbar < 1.0: self._status_message = "Low Vacuum"
        else: self._status_message = "Rough Vacuum / Near Atmospheric"
    ...
    def get_parameter_value(self, parameter_name: str) -> ParameterValue:
        ...
        self._update_simulated_state()
        now = time.time()
        if parameter_name == "current_pressure_mbar":
            return ParameterValue(self._current_pressure_mbar, "mbar", now)
        elif parameter_name == "pump_status":
            return ParameterValue(self._pump_on, "", now)
        raise PECSParameterError(f"Unknown parameter '{parameter_name}' for device {self.device_id}")

    def set_parameter_value(self, parameter_name: str, value: Any) -> bool:
        if not self._is_connected: raise PECSDeviceError(f"Device {self.device_id} disconnected.")
        if parameter_name == "pump_status":
            try:
                self._pump_on = bool(value)
                self._log_to_shms("INFO", f"Pump status set to {self._pump_on} for {self.device_id}.")
                self._update_simulated_state() # Recalculate state immediately
                return True
            except Exception as e:
                raise PECSParameterError(f"Invalid value for pump_status: {e}")
        raise PECSParameterError(f"Parameter '{parameter_name}' is not writable or unknown for {self.device_id}")

    def execute_command(self, command_name: str, args: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        if not self._is_connected: raise PECSDeviceError(f"Device {self.device_id} disconnected.")
        if command_name == "start_pump":
            self.set_parameter_value("pump_status", True)
            return {"success": True, "message": f"Pump started for {self.device_id}."}
        elif command_name == "stop_pump":
            self.set_parameter_value("pump_status", False)
            return {"success": True, "message": f"Pump stopped for {self.device_id}."}
        raise PECSCommandError(f"Unknown command '{command_name}' for device {self.device_id}")

    def _log_to_shms(self, level: str, message: str, data: Optional[Dict[str, Any]] = None): # Helper for consistency
        if self._pecs_callback: # Assuming the callback is to the PECS service which then logs to SHMS
            self._pecs_callback(self.device_id, level.upper() + "_EVENT", {"message": message, **(data or {})})
        else:
            print(f"SimVacuumGauge_LOG [{self.device_id}][{level}]: {message}" + (f" Data: {data}" if data else ""))
