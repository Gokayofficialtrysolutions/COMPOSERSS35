...
class SimulatedQPU_Driver(QuantumDeviceDriver):
    ...
    def execute_gate_commands(self, commands: List[GateCommand]) -> Dict[str, Any]:
        ...
        for cmd in commands:
            ...
            if cmd.gate_type == GateType.I: q_circuit.id(q_indices[0])
            ...
            elif cmd.gate_type == GateType.Y: q_circuit.y(q_indices[0])
            elif cmd.gate_type == GateType.Z: q_circuit.z(q_indices[0])
            elif cmd.gate_type == GateType.H: q_circuit.h(q_indices[0])
            elif cmd.gate_type == GateType.S: q_circuit.s(q_indices[0])
            elif cmd.gate_type == GateType.SDG: q_circuit.sdg(q_indices[0])
            elif cmd.gate_type == GateType.T: q_circuit.t(q_indices[0])
            elif cmd.gate_type == GateType.TDG: q_circuit.tdg(q_indices[0])
            elif cmd.gate_type == GateType.SX: q_circuit.sx(q_indices[0])
            elif cmd.gate_type == GateType.SXDG: q_circuit.sxdg(q_indices[0])
            elif cmd.gate_type == GateType.RZ: q_circuit.rz(params.get('phi', 0.0), q_indices[0])
            elif cmd.gate_type == GateType.RX: q_circuit.rx(params.get('theta', 0.0), q_indices[0])
            elif cmd.gate_type == GateType.RY: q_circuit.ry(params.get('theta', 0.0), q_indices[0])
            elif cmd.gate_type == GateType.P: q_circuit.p(params.get('lambda', 0.0), q_indices[0])
            elif cmd.gate_type == GateType.U:
                q_circuit.u(params.get('theta', 0.0), params.get('phi', 0.0), params.get('lambda', 0.0), q_indices[0])
            elif cmd.gate_type == GateType.CX: q_circuit.cx(q_indices[0], q_indices[1])
            elif cmd.gate_type == GateType.CY: q_circuit.cy(q_indices[0], q_indices[1])
            elif cmd.gate_type == GateType.CZ: q_circuit.cz(q_indices[0], q_indices[1])
            elif cmd.gate_type == GateType.SWAP: q_circuit.swap(q_indices[0], q_indices[1])
            elif cmd.gate_type == GateType.CRX: q_circuit.crx(params.get('theta', 0.0), q_indices[0], q_indices[1])
            elif cmd.gate_type == GateType.CRY: q_circuit.cry(params.get('theta', 0.0), q_indices[0], q_indices[1])
            elif cmd.gate_type == GateType.CRZ: q_circuit.crz(params.get('phi', 0.0), q_indices[0], q_indices[1]) # Qiskit crz uses 'theta' for angle
            elif cmd.gate_type == GateType.CP: q_circuit.cp(params.get('lambda', 0.0), q_indices[0], q_indices[1]) # Qiskit cp uses 'theta'
            elif cmd.gate_type == GateType.CU:
                q_circuit.cu(params.get('theta', 0.0), params.get('phi', 0.0), params.get('lambda', 0.0), params.get('gamma', 0.0), q_indices[0], q_indices[1])
            elif cmd.gate_type == GateType.CCX: q_circuit.ccx(q_indices[0], q_indices[1], q_indices[2])
            elif cmd.gate_type == GateType.CSWAP: q_circuit.cswap(q_indices[0], q_indices[1], q_indices[2])
                ...
                # This method is for appending commands. Measurement execution is separate.
                # However, if a measure command is part of the sequence, it implies classical bits are involved.
                # For now, we assume QHALService.measure_qubits_globally is called after such a sequence.
                # If Qiskit circuit needs explicit classical bits, this would be handled at circuit creation time.
                pass
            elif cmd.gate_type == GateType.RESET:
                 for q_idx in q_indices: q_circuit.reset(q_idx)
            elif cmd.gate_type == GateType.DELAY:
                 q_circuit.delay(params.get('duration'), q_indices[0] if q_indices else None, unit=params.get('unit','dt'))
            else:
                self._qhal_service_callback(self.device_id, "WARNING",
                    {"message": f"Gate {cmd.gate_type.name} not fully implemented in Qiskit sim. Skipping."})

        self._current_qiskit_circuit = q_circuit # Persist changes to the internal circuit
    ...
    def measure_qubits_blocking(self, local_qubit_ids: List[Any], measurement_options: Optional[Dict[str, Any]] = None) -> Dict[Any, int]:
        ...
        try:
            ...
            # Reset self._current_qiskit_circuit after measurement as it's typically a terminal operation for a given run.
            if self._qiskit_qc_class:
                 self._current_qiskit_circuit = self._qiskit_qc_class(self.num_qubits, self.num_qubits)
            return results
        except Exception as e:
            # Reset circuit on error too to prevent corrupted state for next sequence
            if self._qiskit_qc_class:
                 self._current_qiskit_circuit = self._qiskit_qc_class(self.num_qubits, self.num_qubits)
