...
class RMS_Service(AbstractRMS):
    ...
    def _initialize_inventory(self):
        ...
        if self.qhal_service and hasattr(self.qhal_service, 'get_all_global_qubits') and callable(self.qhal_service.get_all_global_qubits):
            try:
                # QHAL's get_all_global_qubits() returns List[AbstractQubit]
                # QHAL's get_qubit_by_global_id(gid).get_properties() returns QubitProperties
                all_qhal_qubits = self.qhal_service.get_all_global_qubits()
                if not all_qhal_qubits and hasattr(self.qhal_service, '_drivers') and not self.qhal_service._drivers:
                     self._log("WARNING", "RMS: QHALService has no registered drivers during inventory init. Qubit inventory will be empty initially.")

                for qubit_obj in all_qhal_qubits:
                    props_dataclass = qubit_obj.get_properties()
                    global_qid = qubit_obj.get_global_id() # Use the method from AbstractQubit

                    self._resource_inventory[global_qid] = ResourceInstance(
                        global_id=global_qid,
                        local_id=props_dataclass.qubit_id, # This is local_id from QubitProperties
                        parent_device_id=global_qid.split("::")[0] if "::" in global_qid else None,
                        type=ResourceType.QUBIT,
                        properties=props_dataclass.__dict__
                    )
                self._log("INFO", f"Initialized qubit inventory from QHAL with {len(all_qhal_qubits)} qubits.")
            except Exception as e:
                self._log("ERROR", f"Failed to initialize qubit inventory from QHAL: {e}", {"exception_type": type(e).__name__})
        else:
            self._log("WARNING", "QHALService not available or does not have 'get_all_global_qubits' method. Qubit inventory will be empty.")
